...
void which(char* program) {
    ...
    int num_pathnames = sizeof(pathnames) / sizeof(pathnames[0]);

    for (int i = 0; i < num_pathnames; i++) {
        char full_path[1024];
        snprintf(full_path, sizeof(full_path), "%s/%s", pathnames[i], program);
        if (access(full_path, 
# 40 "mysh.c" 3 4
                             1
# 40 "mysh.c"
...
# 80 "mysh.c"
    ...
    while(1){
        struct data* capybara = capygetline(
# 92 "mysh.c" 3 4
                                           0
# 92 "mysh.c"
                                                       );
